...
class GameGiftController extends BaseController{
    ...
    */
    public function gift_list($game_id=0){
        ...
        $list = $gift
                ->field("tab_giftbag.id,tab_giftbag.giftbag_name,tab_giftbag.start_time,tab_giftbag.end_time,tab_giftbag.desribe,tab_giftbag.is_unicode,tab_giftbag.unicode_num,tab_game.icon,tab_game.game_name")
        ...
        foreach ($list as $key => $val) {
            ...
            $list[$key]['icon'] = icon_url($val['icon']);
            $list[$key]['now_time'] = NOW_TIME;
            if($list[$key]['is_unicode'] == 1 && $list[$key]['unicode_num'] == 0){
                unset($list[$key]); 
            }
        }
    ...
    */
    public function receive_gift($user_id=0,$gift_id=0,$game_id=0,$game_name=""){
        ...
        if(empty($novice['novice'])){
            $this->set_message(1015,"fail","礼包已被领取完");
        }
        else{
            #将激活码分成数据
            ...
            echo base64_encode(json_encode(array('status'=>200,'receive_status'=>1,'return_code'=>'success','return_msg'=>'领取成功','novice'=>$data_record['novice'])));
            exit();
        }   
    }
    ...
    */
    public function add_gift_record($data = array()){
        ...
        if(!empty($isReceive)){
            echo base64_encode(json_encode(array('status'=>1014,'receive_status'=>0,'return_code'=>'success','return_msg'=>'您已经领取过该礼包','novice'=>$isReceive['novice'])));
            exit();
        }

...
class ThinkController extends AdminController {
    ...
    public function order_lists($model = null, $p = 0,$extend_map = array()){
        ...
        $this->assign('model', $model);
        $this->assign('list_grids', $grids);
        $list_data = $this->array_order_page($count,$arraypage,$data,$row);
        return ['model'=>$model,'grids'=>$grids,'list_data'=>$list_data,'data'=>$data,'count'=>$count,'row'=>$row];
    }
     ...
     * @author [yyh] <[email address]>
     */
    public function data_lists($model = null, $p = 0,$extend_map = array()){
        $model || $this->error('模型名标识必须！');
        $page = intval($p);
        $page = $page ? $page : 1; //默认显示第一页数据
        $arraypage = $page; //默认显示第一页数据
    ...
    public function flists($model = null, $p = 0,$extend_map = array(),$field=''){
        ...
        $row    = intval(C('LIST_ROWS')) ? :10; //empty($model['list_row']) ? 10 : $model['list_row'];

...
class ArticleController extends AdminController {
    ...
    public function index($cate_id = null, $model_id = null, $position = null,$group_id=null){
        ...
        foreach ($grids as $k=> &$value) {
						if ($cate_id == 84 && !(strpos($value,'activity_status') === false)) {
							unset($grids[$k]);
							continue;
						}
            // 字段:标题:链接
    ...
    protected function getDocumentList($cate_id=0,$model_id=null,$position=null,$field=true,$group_id=null){
        ...
        if ( isset($_GET['timestart']) ) {
            $map['update_time'][] = array('egt',strtotime(I('timestart')));
        }
        if ( isset($_GET['timeend']) ) {
            $map['update_time'][] = array('elt',24*60*60 + strtotime(I('timeend')));
        }
        if ( isset($_GET['admin']) ) {
    ...
    public function setStatus($model='Document'){
        if($_REQUEST['status']=='-1'){//删除数据
            if(is_array($_REQUEST['ids'])){
                action_log('pl_del_web_article','article',UID,UID);
            }else{
                action_log('del_web_article','article',UID,UID);
            }
        }
    ...
    public function add(){
        ...
        $Category = M('Category')->where(array('id'=>I('cate_id')))->find();
        if($Category['pid']==81){//所属游戏不用于推广后台
            $this->assign('istui',1);
        }
        //检查该分类是否允许发布
        ...
        // $tuiname=array('tui_zx','tui_gg','tui_about');
        // $category_name = M('Category')->field('name')->find($cate_id);
        // if(in_array($category_name['name'],$tuiname)){
        //     $this->assign('tui',1);
        // }
        //处理结果
    ...
    public function edit(){
        ...
        $Category = M('Category')->where(array('id'=>I('cate_id')))->find();
        $Category = M('Category')->where(array('id'=>I('cate_id')))->find();
        if($Category['pid']==81||in_array($cate_id, [84,85])){//所属游戏不用于推广后台
            $this->assign('istui',1);
        }
        // 获取详细数据 
        $Document = D('Document');
    ...
    public function update(){
        $Category = M('Category')->where(array('id'=>I('category_id')))->find();
        if(!in_array(I('category_id'),array(43,44,45,46,54,55,56,57,68,69,70,71))){//所属游戏仅用于官网app活动公告
            unset($_POST['belong_game']);
        }elseif($_POST['belong_game']==''){
            $this->error('请选择所属游戏');
        }elseif (empty($_POST['cover_id'])){
            $this->error('请上传封面');
        }
        $document   =   D('Document');
        $res = $document->update();
        if(!$res){
            ...
            if($res['id']){
                action_log('update_article','article',UID,UID);
            }else{
                action_log('add_web_article','article',UID,UID);
            }

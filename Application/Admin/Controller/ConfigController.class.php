...
class ConfigController extends AdminController {
    ...
    public function save($config){
        ...
        S('DB_CONFIG_DATA',null);
        action_log('save_config','config',UID,UID);
        $this->success('保存成功！');
    }
    /**
     * 判断是否可以连接上缓存
    ...
    public function check_cache($type,$host){
        if($type==1){
            if(!class_exists('\Redis')){ 
                $this->error('未开启redis拓展，开启缓存失败');
            }
            $redis = new \Redis();
            $res= $redis->connect($host,'6379');
        }elseif ($type==2) {
            if(!class_exists('\Memcache')){ 
                $this->error('未开启memcache拓展，开启缓存失败');
            }
    ...
    public function group() {
        ...
        $list   =   M("Config")->where(array('status'=>1,'group'=>$id,'category'=>0))->field('id,name,title,extra,value,remark,type')->order('sort')->select();
        if($list) {
            $this->assign('list',$list);
        }
        $this->assign('id',$id);

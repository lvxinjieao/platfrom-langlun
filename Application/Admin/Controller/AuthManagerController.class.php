...
class AuthManagerController extends AdminController{
    ...
    public function writeGroup(){
        ...
        if ( $data ) {
            if ( empty($data['id']) ) {
                action_log('add_admin_group','auth',$data['title'],UID);
                $r = $AuthGroup->add();
            }else{
                action_log('update_admin_group','auth',$data['title'],UID);
                $r = $AuthGroup->save();
            }
            if($r===false){
                $this->error('操作失败'.$AuthGroup->getError());
            } else{
                $group_title = M('auth_group')->where(['id'=>$data['id']])->getField('title');
                action_log('update_admin_group_access','auth',$group_title,UID);
    ...
    public function changeStatus($method=null){
        ...
        $id = I('id',0,'intval');
        $title = M('auth_group')->where(['id'=>$id])->getField('title');

        switch ( strtolower($method) ){
            case 'forbidgroup':
                action_log('lock_admin_group','auth',$title,UID);
                $this->forbid('AuthGroup');
                break;
            case 'resumegroup':
                action_log('unlock_admin_group','auth',$title,UID);
                $this->resume('AuthGroup');
                break;
            case 'deletegroup':
                action_log('del_admin_group','auth',$title,UID);
    ...
    public function addToGroup(){
        ...
        if ( $AuthGroup->addToGroup($uid,$gid) ){
            ...
            $group_title = M('auth_group')->where(['id'=>$gid])->getField('title');
            action_log('update_admin_group_member','auth',$group_title,UID);
    ...
    public function removeFromGroup(){
        ...
        if ( $AuthGroup->removeFromGroup($uid,$gid) ){
            ...
            $group_title = M('auth_group')->where(['id'=>$gid])->getField('title');
            action_log('update_admin_group_del_member','auth',$group_title,UID);
    ...
    public function addToCategory(){
        ...
        if ( $AuthGroup->addToCategory($gid,$cid) ){
            ...
            $group_title = M('auth_group')->where(['id'=>$gid])->getField('title');
            action_log('update_admin_group_class','auth',$group_title,UID);

 ...
 * @author 鹿文学 
 */
class BillEvent extends BaseEvent {
    ...
    public function money_list($model = null,$p,$extend=array()) {
        ...
        $count=count($data);
        $to_map=$map;
        if($_REQUEST['status'] == 1 || $_REQUEST['status'] = '' || !isset($_REQUEST['status'])){
            $to_map['status']=1;
            $total =  D($name)
                ->where($to_map)
                ->order($model['order'])
    ...
    public function shou_list($model,$p,$map){
            $model || $this->error('模型名标识必须！');
            $page = intval($p);
            $page = $page ? $page : 1; //默认显示第一页数据
            ...
            $data = M($name,'tab_')
                ...
                ->select();
            /* 查询记录总数 */
            $count = M($name,'tab_')->where($map)->group($group)->select();
            $count = count($count);
             //分页
            if($count > $row){
                $page = new \Think\Page($count, $row);

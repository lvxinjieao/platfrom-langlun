...
class UserEvent extends BaseEvent {
    ...
    public function shou_list($model,$p,$param=array()){
        $model || $this->error('模型名标识必须！');
        $page = intval($p);
        $page = $page ? $page : 1; //默认显示第一页数据
        ...
        $data = M($name,'tab_')
            ...
            ->select();
        /* 查询记录总数 */
        $count = M($name,'tab_')->where($map)->count('distinct('.$group.')');
    ...
    public function check_bill_($model = null,$p,$extend=array(),$param=array()){
        $model || $this->error('模型名标识必须！');
        $page = intval($p);
        $page = $page ? $page : 1; //默认显示第一页数据
        ...
        $sql0 = $user->table("__USER__ as u ")
            ...
            ->group("u.id")       
            ->select(false);
        $sql0 = "select a.promote_id,a.promote_account,a.fgame_id,a.fgame_id as game_id,a.fgame_name,a.fgame_name as game_name,count(a.id) as total_number from ("
            .$sql0
 ...
 public function money_list($model = null,$p,$extend=array()) {
        ...
        $data = $new_model            
            ...
            ->select();
        $count = $new_model->where($map)->count('distinct('.$model['group'].')');
        $total = $new_model

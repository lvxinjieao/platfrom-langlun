...
class DocumentModel extends Model{
    ...
    public function prev($info){
        ...
        /* 返回前一条数据 */
				$data = $this->field('id')->where($map)->order('level desc,id desc')->select();				
				$id = array_column($data,'id');
				$position = array_search($info['id'],$id);
				if (array_key_exists($position-1,$id)) {
					return $this->field(true)->find($id[$position-1]);				
				} else {
					return null;
				}
    }

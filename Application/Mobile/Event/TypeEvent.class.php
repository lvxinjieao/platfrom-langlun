...
class TypeEvent{
	...
	public function qq($token){
		...
		if($data['ret'] == 0){
			$userInfo['type'] = 'QQ';
			$uid = get_union_id($token['access_token']);
			if($uid){
                 $user_old = M('user','tab_')->where(array('openid'=>$token['openid']))->find();
                 if($user_old){
                 	M('user','tab_')->where(array('openid'=>$token['openid']))->save(array('openid'=>$uid));
                 	$userInfo['openid'] = $uid;
                 }else{
                 	$userInfo['openid'] = $uid;
                 }

...
class DocumentModel extends Model{
	...
	public function limlists($category, $order = '`id` DESC', $status = 1, $field = true,$limit=10){
		$map = $this->listMap($category, $status);
		return $this->field($field)->where($map)->order($order)->limit($limit)->select();
	}

	public function listspage($category, $order = '`id` DESC', $status = 1, $field = true,$page=1,$limit=10){
		$map = $this->listMap($category, $status);
	 ...
	 * @return array             查询条件
	 */
	public function listMaps($category, $status = 1, $pos = null){
		/* 设置状态 */
		$map = array('status' => $status, 'pid' => 0);
	...
	public function doc_lists($name='',$game_id='',$p='',$row=10){
		if(!$name){
			return false;
		}
		$category_id = M('category')->where(array('name'=>$name))->find();
		...
		$map['status']=1;
	    $map['deadline'] = array(array('eq',''),array('gt',time()), 'or') ;
	    $map['create_time']=array("BETWEEN",array(0,time()));
	    if($game_id){
	    	$map['sys_document.belong_game']=$game_id;
	    }
	    if($p){
	    	$doc=M('document')
				->where($map)
				->join('sys_document_article on sys_document.id=sys_document_article.id')
				->order('level desc,sys_document.id desc')
				->page($p,$row)
				->select();
			return $doc;
	    }
	...
	public function doc_lists_order($name='',$sort='level desc,sys_document.id desc',$limit=10,$game_id=''){
		if(!$name){
			return false;
		}
		$category_id = M('category')->where(array('name'=>$name))->find();
    ...
    /*获取多个分类文章  */
	public function doc_lists_array($name='',$game_id='',$p='',$row=10){
	    if(!$name){
	        return false;
	    }
	    $maps['name'] = array('in',$name);
	    $category_id = M('category')->where($maps)->getField('id',true);
	    if(!$category_id){
	        return false;
	    }
	...
	public function doc_lists_order_array($name='',$sort='level desc,sys_document.id desc',$limit=10,$game_id=''){
	    if(!$name){
	        return false;
	    }
	    $maps['name'] = array('in',$name);
	...
	/*相关文章  */
	public function alike($id='',$game_id='',$limit=10,$sort='level desc,sys_document.id desc'){
	    if(!$id){
	        return false;
	    }
	    $map['sys_document.id']= ['neq',$id];
	    $map['status']=1;
	    $map['deadline'] = array(array('eq',''),array('gt',time()), 'or') ;
	    $map['create_time']=array("BETWEEN",array(0,time()));
	    if($game_id){

...
class GameModel extends Model{
    ...
    */
    public function game_recommend_limt($recommend_status, $order = '`id` DESC', $status = 1, $field = true,$limit=10){
        $map['tab_game.game_status'] = $status;
        $map['tab_game.recommend_status'] = array('IN',$recommend_status);
        $return=$this
            ->field('tab_game.*,tab_giftbag.game_id')
            ->where($map)
            ->order($order)
            ->limit($limit)
            ->join('tab_giftbag on tab_game.id = tab_giftbag.game_id','left')
            ->group('tab_game.id')
            ->select();
    ...
    //开服表
    public function server($id,$limit=10){
        if($id){
            $map1['tab_game.id']=$id;
        }
        $map1['tab_game.game_status']=1;
        $map1['tab_server.show_status']=1;
        $map1['start_time']=array('egt',time());
        $data=M('server','tab_')
        ->join('tab_game on tab_server.game_id=tab_game.id')
        ->where($map1)
        ->select();
        foreach ($data as $key => $val){
            $data[$key]['play_url']=get_http_url().$_SERVER['HTTP_HOST']."/mobile.php?s=Game/open_game/game_id/".$val['game_id'];
        }
        if(count($data)<$limit){
            $game_keys=array_keys($data);
        }else{
            $game_keys=array_rand($data,$limit);
        }

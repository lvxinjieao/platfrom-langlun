...
class GiftbagModel extends Model{
    ...
    */
    ...
    public function gift_select_list($field='true',$m=array(),$order='sort desc',$limit='')
    {
        // $sql = "select * from (select a.*,c.icon,a.giftbag_name from tab_giftbag as a left join tab_game as c on c.id=a.game_id  where 1 > (select count(b.id) as gift_num from tab_giftbag as b where b.game_id = a.game_id  and a.create_time < b.create_time  order by b.create_time desc )  union  select d.*,d.giftbag_name,count(d.id) as gift_num from tab_giftbag as d group by d.game_id ) as e group by e.game_id";
        $data=M('Giftbag','tab_')
                ->field("tab_giftbag.*,tab_game.icon,tab_game.features")
                ->join("left join tab_game on tab_giftbag.game_id = tab_game.id")
                ->where($m['map'])
                ->group($m['group'])
                ->limit($limit)

...
/* function get_game_type($type = null){
	...
	if (strstr('休闲益智',$type))
		return 0;
	else if (strstr('网络游戏',$type))
		return 1;
	else if (strstr('体育竞速',$type))
		return 2;
	else if (strstr('角色扮演',$type))
		return 3;
	else if (strstr('飞行射击',$type))
		return 4;
	else if (strstr('策略经营',$type))
		return 5;
	else if (strstr('卡片游戏',$type))
		return 6;
	else if (strstr('动作冒险',$type))
		return 7;
	else if (strstr('单机游戏',$type))
		return 8;
	else if (strstr('棋牌天地',$type))
		return 9;
	else 
		return false;
}
...
function get_game_apppid($id)
{
    $map['id']=$id;
    $game=M("game","tab_")->where($map)->find();
    return $game['game_apppid'];
}
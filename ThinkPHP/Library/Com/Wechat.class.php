...
class Wechat {
     ...
     * @param  string $type    消息类型
     */
    public function responsemini($title,$appid,$pagepath,$thumb_media_id,$access_token){
        ...
        $result = $this->http($url,urldecode(json_encode($data)));
        return $result;
    }
    ...
    //curl请求
    protected function http($url, $data = array(), $method = 'POST'){
        $opts = array(
            CURLOPT_TIMEOUT        => 30,

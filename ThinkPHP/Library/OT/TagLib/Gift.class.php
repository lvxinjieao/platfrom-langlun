...
class Gift extends TagLib{
    ...
    protected $tags   =  array(
        ...
        'limits'   => array('attr' => 'name,category,child,identify,limit,field', 'close' => 1), //获取指定分类列表
    );
    ...
    */
    public function _limits($tag, $content){
        ...
        $parse  = '<?php ';
        $parse .= '$__CATE__  = '.$cate.";";
        $parse .= '$__LIST__ = D(\'Giftbag\')->gift_list_limt(';
    ...
    public function _position($tag, $content){
        ...
        $name   = $tag['name'];
        $parse  = '<?php ';
        $parse .= '$__POSLIST__ = D(\'Giftbag\')->position(';
        $parse .= $pos . ',';
        $parse .= $cate . ',';
        $parse .= $limit . ',';
        $parse .= $field . ');';
    ...
    public function _recommend($tag,$content){
        ...
        $parse  = '<?php ';
        $parse .= '$__CATE__  = '.$cate.";";
        $parse .= '$__LIST__ = D(\'Giftbag\')->gift_recommend_limt(';
        $parse .= '$__CATE__, \'`id` DESC\', 1,';
        $parse .= $field .','.$limit.');';
        $parse .= ' ?>';
        $parse .= '<volist name="__LIST__" id="'. $name .'">';
    ...
    public function _partlist($tag, $content){
        ...
        $parse  = '<?php ';
        $parse .= '$__PARTLIST__ = D(\'Giftbag\')->part(' . $id . ',  !empty($_GET["p"])?$_GET["p"]:1, \'' . $field . '\','. $listrow .');';
        $parse .= ' ?>';
